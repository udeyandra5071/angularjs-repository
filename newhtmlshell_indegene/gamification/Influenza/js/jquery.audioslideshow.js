...
var noreward = 0;
var trackscore = [];
var totalquestions = [1,2,3,4];
var traversescore;
var insertscore;
var totalQuestionpush;
var getscore = 0;
var pass_status;
var avatarid = 1;
var setavatar = 1;
...
function send(){
    ...
    //pushme.push({value: value, username : "Udey", Score: "65"});
    //console.log(pushme[0].value,pushme[0].username, pushme[0].Score);
    //$(".username").text("welcome "+pushme[0].username);
...
function send_avatar(){
	...
	var field = document.getElementById("avatarid"),
		value = " Placeholder text ";
  ...
  setavatar = value;
	set('cmi.comments', setavatar);  
	console.log("avartar id"+setavatar);
  $("#avatar").html("<img src='avatars/"+setavatar+".png' />");
}
//avatar ends here//
function set(param, insertscore){

	show("Sending: '" +insertscore +"'");

	var callSucceeded = scorm.set(param, insertscore);
  ...
  (function($) {
      ...
      $.fn.udSynch = function(options) {
        ...
        //End of enabling button//
                  //Initialize scorm and gamification//
                  init();
                      getscore =  scorm.get('cmi.core.score.raw');
                      pass_status =  scorm.get('cmi.core.lesson_status');
                    var getavatar_id = scorm.get('cmi.comments');
                  scorm_meter = scorm.get('cmi.objectives.n.id');
                  console.log("scorm meter "+scorm_meter);
                    var stringLength = getavatar_id.length; // this will be 16
                    var lastChar = getavatar_id.charAt(stringLength - 1);
                  console.log(pass_status);
                  $(".foo").val(scorm_meter);
                  $(".metervalue").text(scorm_meter);
                     $("#score").text("score "+getscore);
                   $('#optionbox').openModal();
                   
                  
                  console.log("get av av value "+lastChar);
                  if(lastChar > 1)
                      {
                         
                          $("#avatar").html("<img src='avatars/"+lastChar+".png' />");
                          $('#optionbox').closeModal();
                          
                      }
                  else{
                        $("#avatar").html("<img src='avatars/1.png' />");
                      //$('#optionbox').openModal();
      ...
      $(".swiper-button-next").bind("click", function(){
          avatarid++;
            $(".avatarbox").val(avatarid);
            //alert(avatarid);
            send_avatar();
             
                   $("#avatar").html("<img src='avatars/"+avatarid+".png' />");
