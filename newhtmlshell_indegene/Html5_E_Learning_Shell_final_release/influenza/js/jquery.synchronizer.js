...
var totalframejoin;
function send(){
// Get frame number to input field.
	var field = document.getElementById("userText"),
		value = " Placeholder text ";
    
	//check empty status of the input field and then push the frame number to input field.
	if(field.value !== null && field.value !== ""){
		value = field.value;
	}
    // Pushing suspend data to an array to send all data together
    trackframe_suspend.push(value);
    // Pushing Bookmarking data to an array to send a bookmark data
    trackframe_lessonlocation.push(value);
    // Sorting suspend data so that there should be no duplicate frame numbers.
    trackframe_suspend.sort();
    // checking the result in console.
    console.log(trackframe_lessonlocation);
    console.log(trackframe_suspend);
    //After sorting all suspend data then find the unique data and initialize to the variable..
    trackframe_suspend_two = jQuery.unique(trackframe_suspend);
    console.log(trackframe_suspend_two);
    //after all join all data together to send to lms.
    join_data = trackframe_suspend_two.join();  
    console.log(join_data);
    //Here we are passing the suspend joined data to lms using set().
	set('cmi.suspend_data', join_data);  
    //Here we are passing the bookmarking data to lms using set().
	set('cmi.core.lesson_location', value); 
    totalframejoin = totalframe_tracker.join();
    console.log(totalframejoin);
  //Checking the length of join data and total frame exist in module and then setting the status complete or incomplete            
    if((join_data.length) == (totalframejoin.length))
        {
            //If equal the set to completed
    ...
    show("<hr>");  
          alert("Congratulations you have completed your course");
        }
...
//showing a popup and load the resumed frame
...
// check the number of frames and set the status of the course.
   if((value.length) == 12)
   {
...
// Save the data to lms db while exiting the course.
function end(){
...
// Shows the data in debugging console.
function show(msg){
...
//Bookmark popup button.
  ...
  //    $(".modal-close").bind("click",function(){
      ...
      //Parent menu icon which will show the menu of the module - Glossary/reference etc
      ...
      //audio transcript show and hide feature
      ...
      //Table of content show/hide feature
...
// Load audio transcript on the basis of frame number
...
// Timeline feature with click event
