...
function SCORMFinish(scoStartTime) {
	...
	if (top) {
	try{
	window.opener.close1();
	parent.window.opener.close1();
	setTimeout('parent.window.opener.top.close()', 500);}catch(e){};
...
function chapterViewed(chapterNum) {
  ...
  var a=_cmiData.suspendData.split(",")
  if(a[a.length-1]==1)
  {
	allComplete = true;
  }
...
function scormCurrentFrame(chapterNum){
	_cmiData.lessonLocation = chapterNum+"";
	  doLMSSetValue("cmi.core.lesson_location", _cmiData.lessonLocation);	
}
